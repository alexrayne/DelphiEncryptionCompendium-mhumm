...
type
  ...
  public
    ...
    procedure mic_setup( const attr; asize : LongWord ;
                             dsize: LongWord;
                             msize: byte;
...
implementation
uses DECutil, DECTypes, DECCipherBase;
...
// @result: Buffer = B0
procedure TDEC_CCM.mic_setup(const attr; asize : LongWord ;
    ...
    if (asize > 0) then begin
      ...
      if (asize < $ff00) then begin
        ...
        if (asize <= $ffffffff) then begin
          tmp[1] := $fe;
          SwapUInt32Buffer(asize, tmp[2], 4);
          pos := 6;
        end
        else begin
          tmp[1] := $ff;
